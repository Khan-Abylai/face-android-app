...
int FaceDetection::loadModel(AAssetManager* mgr){
    retina.clear();

    ncnn::set_cpu_powersave(2);
    ncnn::set_omp_num_threads(4);
    retina.opt = ncnn::Option();

    retina.opt.use_vulkan_compute = true;

    int ret = retina.load_param(mgr, "face.param");
    if (ret) {
        __android_log_print(ANDROID_LOG_ERROR, "load_param_error",
                            "Failed to load the model parameters");
    }
    ret = retina.load_model(mgr, "face.bin");
    if (ret) {
...
static void nms(std::vector<bbox> &input_boxes, float NMS_THRESH)
{
...
static void create_anchor(std::vector<box> &anchor, int w, int h)
{
...
static bool cmp(bbox a, bbox b) {
    if (a.s > b.s)
        return true;
    return false;
}

void FaceDetection::Detect(cv::Mat& rgb, std::vector<bbox>& boxes)
{
    cv::Mat bgr;
...
bbox FaceDetection::chooseOneFace(std::vector<bbox> &input_boxes, float scale){
    bbox result;
    float maxArea = -1.0;
...
//    cv::cvtColor(aligned_face, aligned_face, cv::COLOR_BGR2RGBA);
    return aligned_face;
}

cv::Mat FaceDetection::alignFace(cv::Mat img, bbox input_box){
...
int FaceDetection::draw(cv::Mat &rgb, bbox inputBox, cv::Mat face) {

    for (size_t i = 0; i < 5; i++)
    {
        cv::circle(rgb, cv::Point2f(inputBox.point[i]._x, inputBox.point[i]._y), 2,cv::Scalar(255,255,0),-1);
    }
    cv::rectangle(rgb, cv::Rect(cv::Point(inputBox.x1, inputBox.y1), cv::Size(abs(inputBox.x2 - inputBox.x1), abs(inputBox.y2-inputBox.y1))), cv::Scalar(0,0,255));

    cv::Mat insetImage = rgb(cv::Rect(0, 480-face.rows, face.cols, face.rows));

    cv::putText(rgb, to_string(inputBox.s), cv::Point(inputBox.x1, inputBox.y1 + 5), cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0,255,0));
    face.copyTo(insetImage);
