...
public class CameraRenderer extends CameraGLRendererBase {
    ...
    protected synchronized void openCamera(int id) {
        ...
        if (id == CameraBridgeViewBase.CAMERA_ID_ANY) {
            Log.d(LOGTAG, "Trying to open ndkcamera with old open()");
            try {
                mCamera = Camera.open();
            }
            catch (Exception e){
            ...
            if(mCamera == null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                ...
                for (int camIdx = 0; camIdx < Camera.getNumberOfCameras(); ++camIdx) {
                    Log.d(LOGTAG, "Trying to open ndkcamera with new open(" + camIdx + ")");
                    try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                ...
                if (mCameraIndex == CameraBridgeViewBase.CAMERA_ID_BACK) {
                    Log.i(LOGTAG, "Trying to open BACK ndkcamera");
                    ...
                    Log.i(LOGTAG, "Trying to open FRONT ndkcamera");
                    Camera.CameraInfo cameraInfo = new Camera.CameraInfo();
                    for (int camIdx = 0; camIdx < Camera.getNumberOfCameras(); ++camIdx) {
                        Camera.getCameraInfo( camIdx, cameraInfo );
                ...
                if (localCameraIndex == CameraBridgeViewBase.CAMERA_ID_BACK) {
                    Log.e(LOGTAG, "Back ndkcamera not found!");
                } else if (localCameraIndex == CameraBridgeViewBase.CAMERA_ID_FRONT) {
                    Log.e(LOGTAG, "Front ndkcamera not found!");
                } else {
                    Log.d(LOGTAG, "Trying to open ndkcamera with new open(" + localCameraIndex + ")");
                    try {
        ...
        if(mCamera == null) {
            Log.e(LOGTAG, "Error: can't open ndkcamera");
            return;
        }
    ...
    public synchronized void setCameraPreviewSize(int width, int height) {
        ...
        if (psize.size() > 0) {
            ...
            for (Size size : psize) {
                ...
                Log.d(LOGTAG, "checking ndkcamera preview size: "+w+"x"+h);
                if ( w <= width && h <= height &&
