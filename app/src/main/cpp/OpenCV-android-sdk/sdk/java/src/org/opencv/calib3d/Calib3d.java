...
public class Calib3d {
    ...
    /**
     * Decomposes a projection matrix into a rotation matrix and a ndkcamera intrinsic matrix.
     *
     * @param projMatrix 3x4 input projection matrix P.
     * @param cameraMatrix Output 3x3 ndkcamera intrinsic matrix \(\cameramatrix{A}\).
     ...
     * @param cameraMatrix Input ndkcamera intrinsic matrix \(\cameramatrix{A}\) .
     * @param distCoeffs Input vector of distortion coefficients
     ...
     * using the perspective projection model \( \Pi \) and the ndkcamera intrinsic parameters matrix \( \bf{A} \):
     *
     * \(
     *   \begin{align*}
     *   \begin{bmatrix}
    ...
    /**
     * Finds an initial ndkcamera intrinsic matrix from 3D-2D point correspondences.
     *
    ...
    /**
     * Finds the ndkcamera intrinsic and extrinsic parameters from several views of a calibration
     * pattern.
     *
    ...
    /**
     * Finds the ndkcamera intrinsic and extrinsic parameters from several views of a calibration pattern.
     *
    ...
    /**
     * Computes useful ndkcamera characteristics from the ndkcamera intrinsic matrix.
     *
    ...
    /**
     * Calibrates a stereo ndkcamera set up. This function finds the intrinsic parameters
     ...
     * ( \(R_1\),\(T_1\) ) and (\(R_2\),\(T_2\)), respectively, for a stereo ndkcamera where the
    ...
    /**
     * Computes rectification transforms for each head of a calibrated stereo ndkcamera.
     *
     ...
     * <ul>
     *   <li>
     *    <b>Horizontal stereo</b>: the first and the second ndkcamera views are shifted relative to each other
     ...
     * <ul>
     *   <li>
     *    <b>Vertical stereo</b>: the first and the second ndkcamera views are shifted relative to each other
    ...
    /**
     * Computes a rectification transform for an uncalibrated stereo ndkcamera.
     *
    ...
    /**
     * Returns the new ndkcamera intrinsic matrix based on the free scaling parameter.
     *
     ...
     * expressed in the target frame to the ndkcamera frame (\(_{}^{c}\textrm{T}_t\)).
     * This is a vector ({@code vector&lt;Mat&gt;}) that contains the rotation, {@code (3x3)} rotation matrices or {@code (3x1)} rotation vectors,
     ...
     * expressed in the ndkcamera frame to the gripper frame (\(_{}^{g}\textrm{T}_c\)).
     ...
     * expressed in the world frame to the ndkcamera frame (\(_{}^{c}\textrm{T}_w\)).
     * This is a vector ({@code vector&lt;Mat&gt;}) that contains the rotation, {@code (3x3)} rotation matrices or {@code (3x1)} rotation vectors,
     ...
     * expressed in the gripper frame to the ndkcamera frame (\(_{}^{c}\textrm{T}_g\)).
    ...
    /**
     * Recovers the relative ndkcamera rotation and the translation from an estimated essential
     ...
     * @param cameraMatrix Input ndkcamera matrix \(A = \vecthreethree{f_x}{0}{c_x}{0}{f_y}{c_y}{0}{0}{1}\) .
     * @param distCoeffs Input vector of distortion coefficients
     * \((k_1, k_2, p_1, p_2[, k_3[, k_4, k_5, k_6[, s_1, s_2, s_3, s_4[, \tau_x, \tau_y]]]])\)
     * of 4, 5, 8, 12 or 14 elements. If the vector is NULL/empty, the zero distortion coefficients are assumed.
     ...
     * @param newCameraMatrix New ndkcamera matrix \(A'=\vecthreethree{f_x'}{0}{c_x'}{0}{f_y'}{c_y'}{0}{0}{1}\).
    ...
    /**
     * Returns the default new ndkcamera matrix.
     *
     ...
     * @param P New ndkcamera matrix (3x3) or new projection matrix (3x4) \(\begin{bmatrix} {f'}_x &amp; 0 &amp; {c'}_x &amp; t_x \\ 0 &amp; {f'}_y &amp; {c'}_y &amp; t_y \\ 0 &amp; 0 &amp; 1 &amp; t_z \end{bmatrix}\). P1 or P2 computed by
    ...
    /**
     * Estimates new ndkcamera intrinsic matrix for undistortion or rectification.
     *
     *     @param K Camera intrinsic matrix \(cameramatrix{K}\).
    ...
    /**
     * Performs ndkcamera calibaration
     *
     *     @param objectPoints vector of vectors of calibration pattern points in the calibration pattern
     *     coordinate space.
    ...
    /**
     * Stereo rectification for fisheye ndkcamera model
     *
